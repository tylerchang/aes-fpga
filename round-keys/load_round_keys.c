...
void print_usage(const char* progname) {
    printf("Usage: %s <key_hex> [config_mode]\n", progname);
    printf("  <key_hex>: 32 hex characters representing the 16-byte initial key\n");
    printf("  [config_mode]: Optional. Endianness and word order configuration (1-4):\n");
    printf("      1 = Big endian, normal word order\n");
    printf("      2 = Big endian, reverse word order (default)\n");
    printf("      3 = Little endian, normal word order\n");
    printf("      4 = Little endian, reverse word order\n");
    printf("Example: %s 2B7E151628AED2A6ABF7158809CF4F3C 2\n", progname);
}

// AES S-box (copied from generate-round-keys.c)
...
// Key schedule functions (copied from generate-round-keys.c)
...
int main(int argc, char* argv[]) {
    ...
    int config_mode = 2; // Default: Big endian, reverse word order
    ...
    if (ioctl(fd, ROUND_KEYS_SET_CONFIG, &config_mode) < 0) {
